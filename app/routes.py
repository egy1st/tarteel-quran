...
def loop():
    ...
    #open text file
    text_file = open("C:\\Users\\moham\\Documents\\GitHub\\tarteel-quran\\data.txt", "w")
...
def hefz():
    ...
    if  request.method == 'POST':
        ...
        hefz_mode = request.form.getlist('cohort_radio')[0]
        if hefz_mode == "ON":
            radioCohort["ON"] = 'checked=""'
            radioCohort["OFF"] = ''
            radioCohort["Hefz_Display"] = "block"
            radioCohort["Listen_Display"] = "none"
        else:
            radioCohort["OFF"] = 'checked=""'
            radioCohort["ON"] = ''
            radioCohort["Listen_Display"] = "block"
            radioCohort["Hefz_Display"] = "none"
            
        print('hefz_mode hefz_mode', hefz_mode)
        print(radioCohort)
    ...
    if repeat_hefz is None or repeat_hefz == "":
        repeat_hefz = "0"
        
    if repeat_prepare is None or repeat_prepare == "":
        repeat_prepare = "0"
    ...
    if hefz_mode == "" :
        hefz_mode = "OFF" #listenng is default
          
    if  safah == "":

        # print('img_res', img_res,'safah', type(safah), safah==None , safah=="", safah == 0, 'hefz_mode', hefz_mode)
        ...
        if hefz_mode == 'OFF' :  #listening mode
            ...
            safah = int(safah_data[0]['page'])
            # print('hefz_mode', hefz_mode, 'safah', safah)
    ...
    else: # safah is not empty  
        safah_data= db_helper.get_data_for_safah(safah)
        safah = int(safah)
        # print('safah init', safah)
    ...
    #print('img_res', img_res,'safah', type(safah), safah==None , safah=="", safah == 0)
    ...
    if hefz_mode == 'ON' :  #Hefz mode
         ...
         i_init = int(cohort[1]['ayah'])
         i_to = int(cohort[n_cohort]['to_ayah']) + 1       
         ...
         for ct in range(1, n_cohort+1):
             ...
             for r in range (1, int(repeat)+1):             
                   ...
                   for i in range (i_init, i_to):
                       ...
                       _ayah_fill = uniformNumber(str(i))
                       _ayahID = cohort_surah + ":" + str(i)
                       if r == 1 : # repeat Ayah in first loop only
                            inner_looper = int(ayah_repeat)
                       else:
                             inner_looper = 1
                       
                       for a in range(1, inner_looper+1):
                           ayah_i =  _surahfill + _ayah_fill + ".mp3"
                           ayahDIC = {}
                           ayahDIC['ayahID'] = _ayahID 
                           ayahDIC['audio'] = ayah_i
             ...
             if y != 1 : # do not accmulate first cohort
                
                for x in range(int(repeat_hefz)):
                      
                           
                  for ct_acc in range(1, y+1):
             
                       i_init = int(cohort[ct_acc]['ayah'])
                       i_to = int(cohort[ct_acc]['to_ayah']) + 1
    ...
    elif hefz_mode == 'OFF' :  # Listening mode
